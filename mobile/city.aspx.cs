    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            for (int i = 0; i < zm.Length - 1; i++)
            {
                ...
                if (zm[i] == "C")
                {
                    ioliu += "<a href='city.aspx?cname=重庆市' style='font-size:1.1em; color:#7724f5; text-decoration:none; float:left; margin:15px;'><font face='微软雅黑'>重庆市</font></a>";
                }
                foreach (DataRow dt in dal.select("select * from address_s where dbo.f_GetPy(sname) like '" + zm[i] + "%' order by spai desc").Rows)
                {
                    ioliu += "<a href='city.aspx?cname=" + dt["sname"].ToString() + "' style='font-size:1.1em; color:#7724f5; text-decoration:none; float:left; margin:15px;'><font face='微软雅黑'>" + dt["sname"].ToString() + "</font></a>";
                }
            }
